...
export default async function handler(req, res) {
  ...
  // ── Kontaktu/pieteikumu paziņojums administratoram ──────────────────────────
  if (req.body.type === 'contact') {
    const { to, no_epasta, vards, saturs, tips } = req.body
    if (!to || !saturs) return res.status(400).json({ error: 'Nav adresāta vai satura' })
    const apiKey = process.env.RESEND_API_KEY
    if (!apiKey) return res.status(500).json({ error: 'RESEND_API_KEY nav iestatīts' })
    const tipNos = { pieteikums: 'Pieteikums', zina: 'Ziņa', sudziba: 'Sūdzība' }[tips] || 'Ziņa'
...
<tr><td style="background:#1b5e20;padding:20px 28px"><div style="color:#fff;font-size:18px;font-weight:700">🌲 Meža Tirgus — jauns ${tipNos.toLowerCase()}</div></td></tr>
<tr><td style="padding:28px">
<table width="100%" cellpadding="0" cellspacing="0">
<tr><td style="padding:6px 0;font-size:13px;color:#6c757d;width:120px">No:</td><td style="padding:6px 0;font-size:14px;color:#212529"><b>${vards || '—'}</b> &lt;${no_epasta}&gt;</td></tr>
<tr><td style="padding:6px 0;font-size:13px;color:#6c757d">Tips:</td><td style="padding:6px 0;font-size:14px;color:#212529">${tipNos}</td></tr>
</table>
<div style="margin-top:16px;padding:16px;background:#f8f9fa;border-radius:8px;border-left:3px solid #2e7d32">
<div style="font-size:13px;color:#6c757d;margin-bottom:8px">Ziņas saturs:</div>
<div style="font-size:14px;color:#212529;line-height:1.6;white-space:pre-wrap">${saturs.replace(/</g,'&lt;').replace(/>/g,'&gt;')}</div>
</div>
</td></tr>
</table></td></tr></table>
</body></html>`
    try {
      const r = await fetch('https://api.resend.com/emails', {
        ...
        body: JSON.stringify({
          ...
          to:      [to],
          subject: `Jauns ${tipNos.toLowerCase()} no ${vards || no_epasta} — Meža Tirgus`,
          html,
        }),
      })
      const d = await r.json()
      if (!r.ok) return res.status(r.status).json({ error: d.message || 'Resend kļūda' })
