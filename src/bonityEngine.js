...
// 4. tabula — Audžu bonitātes (Latvijas VMD normatīvi)
// Formāts: { bonitāte: [maksimālais_augstums, minimālais_augstums] }
...
const bonitateSkuju = {
  ...
  30:  {Ia:[16,14], I:[13,12], II:[11,10], III:[9,8],   IV:[7,6],   V:[5,4],   Va:[3,2]},
...
export function getBonitate(suga, vecums, h) {
  ...
  // Noapaļo vecumu uz tuvāko tabulas rindu (zemāk vai vienādi)
  ...
  // Pārbauda no labākās uz sliktāko — atgriež pirmo klasi kuras min <= h
  // Tas novērš "caurumus" starp klasēm un apgrieztu [max,min] problēmas
  const bonOrder = ["Ia","I","II","III","IV","V","Va"]
  for (const bon of bonOrder) {
    const range = rinda[bon]
    if (!range) continue
    const min = range[1] // [max, min] formāts
    if (h >= min) return bon
  }
  return "Va"
}
