      ...
      <div style={{ background: "white", borderRadius: 10, maxWidth: 520, width: "100%", maxHeight: "85vh", overflow: "auto", padding: 28, boxShadow: "0 8px 32px rgba(0,0,0,0.3)" }}>
        ...
        {isIzlicejs && (
          <div style={{ fontSize: 13, color: "#333", lineHeight: 1.7 }}>
            ...
            <div style={{ background: "#fff8e1", border: "1px solid #f9a825", borderRadius: 6, padding: "10px 14px", fontSize: 12, marginTop: 8 }}>
        ...
        {isDaliba && (
          <div style={{ fontSize: 13, color: "#333", lineHeight: 1.7 }}>
            <p><b>Reģistrējoties dalībai apliecini:</b></p>
            <ul style={{ paddingLeft: 20, margin: "8px 0" }}>
        ...
        {!isIzlicejs && !isDaliba && (
          <div style={{ fontSize: 13, color: "#333", lineHeight: 1.7 }}>
            <p><b>Pirms solīšanas atgādinājums:</b></p>
            <ul style={{ paddingLeft: 20, margin: "8px 0" }}>
...
function FotoUpload({ fotos, setFotos }) {
  const handleFoto = (e) => {
    ...
    files.forEach(file => {
      const reader = new FileReader()
      reader.onload = (ev) => {
        setFotos(prev => [...prev, { nosaukums: file.name, data: ev.target.result }])
      }
  ...
  return (
    <div style={{ marginBottom: 16, padding: 16, background: "#f9f9f9", borderRadius: 8, border: "1px dashed #ccc" }}>
      <label style={{ fontSize: 11, fontWeight: "bold", display: "block", marginBottom: 6 }}>📸 Foto (cirsma, ceļš, krautuve):</label>
      <input type="file" accept="image/*" multiple onChange={handleFoto} style={{ fontSize: 13 }} />
      {fotos.length > 0 && (
        <div style={{ display: "flex", flexWrap: "wrap", gap: 8, marginTop: 10 }}>
          {fotos.map((f, i) => (
            <div key={i} style={{ position: "relative" }}>
              <img src={f.data} alt={f.nosaukums} style={{ width: 80, height: 60, objectFit: "cover", borderRadius: 4, border: "1px solid #ccc" }} />
...
function IzsoleForm({ user, onSaglabat, onAtcelt }) {
  ...
  const [sakums, setSakums] = useState(() => {
...
const [pdfBase64, setPdfBase64] = useState(null)
  ...
  const lkmGeoUrl = kadastrs ? `https://www.lvmgeo.lv/kartes?cadastre=${kadastrs}` : "https://www.lvmgeo.lv/kartes"

 const handlePDF = async (e) => {
    const file = e.target.files[0]
    if (!file) return
    setPdfFile(file)
    // Saglabā PDF kā base64
  ...
  return (
    <>
      ...
      <div style={{ background: "white", border: "2px solid #e65100", borderRadius: 10, padding: 24, marginBottom: 16 }}>
        ...
        {kludas && <div style={{ background: "#ffebee", color: "#c62828", padding: 8, borderRadius: 4, marginBottom: 12, fontSize: 12 }}>{kludas}</div>}
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 12, marginBottom: 16 }}>
          <div>
            ...
            <input type="number" value={sakumcena} onChange={e => setSakumcena(e.target.value)} placeholder="0"
              style={{ width: "100%", padding: 6, border: "1px solid #ccc", borderRadius: 4, fontSize: 13, boxSizing: "border-box" }} />
            {sakumcena && <div style={{ fontSize: 11, color: "#888", marginTop: 2 }}>Solīšanas solis: {getSolis(parseFloat(sakumcena))} €</div>}
          </div>
          <div>
            <label style={{ fontSize: 11, fontWeight: "bold", display: "block", marginBottom: 3 }}>Izsole sākas:</label>
            <input type="date" value={sakums} onChange={e => setSakums(e.target.value)}
              style={{ width: "100%", padding: 6, border: "1px solid #ccc", borderRadius: 4, fontSize: 13, boxSizing: "border-box" }} />
          </div>
          <div>
        ...
        <div style={{ marginBottom: 16 }}>
          <label style={{ fontSize: 11, fontWeight: "bold", display: "block", marginBottom: 6 }}>Redzamība:</label>
        ...
        <div style={{ marginBottom: 16, padding: 16, background: "#f9f9f9", borderRadius: 8, border: "1px dashed #ccc" }}>
          <label style={{ fontSize: 11, fontWeight: "bold", display: "block", marginBottom: 6 }}>📎 PDF pielikums (inventarizācija vai dastojums):</label>
          <input type="file" accept=".pdf" onChange={handlePDF} style={{ fontSize: 13 }} />
          {lādē && <div style={{ marginTop: 8, fontSize: 13, color: "#1565c0" }}>⏳ Nolasa PDF...</div>}
          {pdfFile && !lādē && (
            <div style={{ marginTop: 8, fontSize: 12, color: "#225522" }}>
              ✓ {pdfFile.name}
              {pdfTips === "dastojums" && <span style={{ marginLeft: 8, background: "#e8f5e9", padding: "2px 8px", borderRadius: 10, fontSize: 11 }}>🌲 Dastojums atpazīts</span>}
              ...
              {pdfTips === "nezinams" && <span style={{ marginLeft: 8, background: "#fff8e1", padding: "2px 8px", borderRadius: 10, fontSize: 11 }}>📄 PDF pievienots</span>}
            </div>
          )}
        </div>
        ...
        {analīze && analīze.tips === "dastojums" && Object.keys(sortimenti).length > 0 && (
          <div style={{ marginBottom: 16, padding: 16, background: "#f0f8f0", border: "2px solid #225522", borderRadius: 8 }}>
            <div style={{ fontWeight: "bold", color: "#225522", marginBottom: 10, fontSize: 14 }}>
              📊 Analīzes rezultāti
              <span style={{ fontSize: 11, color: "#888", fontWeight: "normal", marginLeft: 8 }}>(precizitāte atkarīga no inventarizācijas datuma)</span>
            </div>
            ...
            <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 12, marginBottom: 8 }}>
              <thead><tr style={{ background: "#225522", color: "white" }}>
              ...
              <tbody>
                {Object.keys(sortimenti).filter(k => sortimenti[k] > 0).map((k, i) => (
                  <tr key={k} style={{ background: i % 2 === 0 ? "white" : "#f9f9f9" }}>
                    <td style={{ padding: "3px 8px", borderBottom: "1px solid #e0e0e0" }}>{SORT_NOSAUKUMI[k]}</td>
              ...
              <tfoot>
                <tr style={{ background: "#f0f8f0" }}><td style={{ padding: "4px 8px", fontWeight: "bold" }}>Kopā</td><td style={{ padding: "4px 8px", textAlign: "right", fontWeight: "bold" }}>{kopKub.toFixed(1)} m³</td><td></td><td style={{ padding: "4px 8px", textAlign: "right", fontWeight: "bold" }}>{sortVert.toFixed(0)} €</td></tr>
                <tr style={{ background: "#fff3e0" }}><td colSpan={3} style={{ padding: "4px 8px", fontSize: 11, color: "#e65100" }}>Izmaksas ({izmaksas.zaglesana + izmaksas.pievesana} €/m³ × {kopKub.toFixed(1)} m³)</td><td style={{ padding: "4px 8px", textAlign: "right", color: "#c62828", fontWeight: "bold" }}>-{izmaksasKopa.toFixed(0)} €</td></tr>
                <tr style={{ background: "#e8f5e9" }}><td colSpan={3} style={{ padding: "6px 8px", fontWeight: "bold", color: "#225522", fontSize: 13 }}>🌲 Krautuves vērtība</td><td style={{ padding: "6px 8px", textAlign: "right", fontWeight: "bold", color: "#225522", fontSize: 15 }}>{krautuveVert.toFixed(0)} €</td></tr>
              </tfoot>
            </table>
            <div style={{ padding: "10px 14px", background: "white", border: "1px solid #c8dcc0", borderRadius: 6 }}>
              <label style={{ display: "flex", alignItems: "center", gap: 10, cursor: "pointer", fontSize: 13 }}>
                ...
                <span><b>Pievienot aprēķinus izsoles sludinājumam</b>
                  <span style={{ display: "block", fontSize: 11, color: "#888", marginTop: 2 }}>Pircēji redzēs oriģinālo PDF un aprēķina rezultātus</span>
                </span>
              </label>
            </div>
          </div>
        ...
        <div style={{ display: "flex", gap: 8 }}>
          <button onClick={validet} style={{ padding: "10px 24px", background: "#e65100", color: "white", border: "none", borderRadius: 6, cursor: "pointer", fontWeight: "bold", fontSize: 14 }}>
            🏷 Izlikt izsolē
          </button>
          <button onClick={onAtcelt} style={{ padding: "10px 16px", background: "#888", color: "white", border: "none", borderRadius: 6, cursor: "pointer" }}>
            Atcelt
  ...
  return (
    <div style={{ position: "fixed", top: 0, left: 0, width: "100%", height: "100%", background: "rgba(0,0,0,0.6)", zIndex: 2000, display: "flex", alignItems: "center", justifyContent: "center", padding: 16 }}>
      <div style={{ background: "white", borderRadius: 10, maxWidth: 400, width: "100%", padding: 28 }}>
        <h3 style={{ color: "#225522", marginTop: 0 }}>💰 Solīt</h3>
        <div style={{ background: "#f0f8f0", borderRadius: 6, padding: "10px 14px", marginBottom: 16, fontSize: 13 }}>
          <div>Sākumcena: <b>{izsole.sakumcena.toLocaleString()} €</b></div>
          <div>Lielākais solījums: <b style={{ color: "#e65100" }}>{lielakais.toLocaleString()} €</b></div>
          <div>Minimālais nākamais: <b style={{ color: "#225522" }}>{minSumma.toLocaleString()} €</b></div>
          <div style={{ fontSize: 11, color: "#888" }}>Solīšanas solis: {solis} €</div>
          {manamsPedjais && <div style={{ marginTop: 4 }}>Mans pēdējais: <b style={{ color: "#1565c0" }}>{manamsPedjais.toLocaleString()} €</b></div>}
        </div>
        ...
        <input type="number" value={summa} onChange={e => setSumma(e.target.value)}
          ...
          style={{ width: "100%", padding: 8, border: "2px solid #225522", borderRadius: 6, fontSize: 16, marginTop: 4, marginBottom: 8, boxSizing: "border-box" }} autoFocus />
        <div style={{ display: "flex", gap: 6, flexWrap: "wrap", marginBottom: 16 }}>
          {[minSumma, minSumma + solis, minSumma + solis * 2].map(s => (
            <button key={s} onClick={() => setSumma(String(s))}
              style={{ padding: "4px 10px", background: "#f0f8f0", border: "1px solid #225522", borderRadius: 4, cursor: "pointer", fontSize: 12, color: "#225522" }}>
              {s.toLocaleString()} €
            </button>
          ))}
        </div>
...
function IzsoleKarte({ izsole, user, onAtjaunot }) {
  ...
  const solit = (summa) => {
    ...
    const lielakaisPasreiz = soljumi.filter(s => s.izsolId === izsole.id).reduce((max, s) => Math.max(max, s.summa), izsole.sakumcena)
    // Laika pagarinājums — ja solīts pēdējās 5 minūtēs pirms beigām
  ...
  const fazeBadge = () => {
    if (faze === "gaida") return <span style={{ background: "#e3f2fd", color: "#1565c0", padding: "2px 8px", borderRadius: 12, fontSize: 11, marginLeft: 6 }}>⏳ {lidzSakumam(izsole.sakums)}</span>
    if (faze === "beigusies") return <span style={{ background: "#f5f5f5", color: "#888", padding: "2px 8px", borderRadius: 12, fontSize: 11, marginLeft: 6 }}>Beigusies</span>
    return <span style={{ background: "#e8f5e9", color: "#225522", padding: "2px 8px", borderRadius: 12, fontSize: 11, marginLeft: 6 }}>🟢 Aktīva</span>
  }
  ...
  return (
    <>
      ...
      <div style={{ background: "white", border: `2px solid ${faze === "aktiva" ? "#e65100" : "#ccc"}`, borderRadius: 10, padding: 20, boxShadow: "0 2px 8px rgba(0,0,0,0.08)" }}>
        {/* Header */}
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 10 }}>
          <div style={{ display: "flex", flexWrap: "wrap", gap: 4, alignItems: "center" }}>
            <span style={{ background: "#fff3e0", color: "#e65100", padding: "2px 8px", borderRadius: 12, fontSize: 11, fontWeight: "bold" }}>🏷 Izsole</span>
            {fazeBadge()}
            {izsole.publiska === false && <span style={{ background: "#e8eaf6", color: "#534AB7", padding: "2px 8px", borderRadius: 12, fontSize: 11, marginLeft: 4 }}>🔒 Privāta</span>}
          </div>
          <div style={{ textAlign: "right", fontSize: 12, color: "#888" }}>
            <div>{izsole.datums}</div>
            {faze === "aktiva" && <div style={{ color: "#e65100", fontWeight: "bold" }}>⏱ {laiks.teksts}</div>}
          </div>
        </div>

        <h3 style={{ margin: "0 0 6px", color: "#225522", fontSize: 16 }}>{izsole.nosaukums}</h3>

        <div style={{ display: "flex", gap: 12, flexWrap: "wrap", marginBottom: 8, fontSize: 12, color: "#555" }}>
          ...
          {izsole.kadastrs && (
            <a href={`https://www.lvmgeo.lv/kartes?cadastre=${izsole.kadastrs}`} target="_blank" rel="noreferrer"
              style={{ color: "#1565c0", textDecoration: "none", fontSize: 12 }}>
              🗺 {izsole.kadastrs}
            </a>
          )}
          {izsole.pdfNosaukums && izsole.pdfBase64 && (
  <a href={izsole.pdfBase64} download={izsole.pdfNosaukums}
    style={{ color: "#e65100", textDecoration: "none", fontSize: 12 }}>
    📄 {izsole.pdfNosaukums} ⬇
  </a>
)}
{izsole.pdfNosaukums && !izsole.pdfBase64 && (
  <span style={{ color: "#e65100" }}>📄 {izsole.pdfNosaukums}</span>
)}
        </div>

        {/* Datumi */}
        <div style={{ display: "flex", gap: 16, marginBottom: 10, fontSize: 12, color: "#666" }}>
          {izsole.sakums && <span>Sākas: <b>{new Date(izsole.sakums).toLocaleDateString("lv-LV")}</b></span>}
          <span>Beidzas: <b>{new Date(izsole.beigas).toLocaleDateString("lv-LV")}</b></span>
        </div>
        ...
        {izsole.fotos?.length > 0 && (
          <div style={{ display: "flex", gap: 8, marginBottom: 12, flexWrap: "wrap" }}>
            {izsole.fotos.map((f, i) => (
              <img key={i} src={f.data} alt={f.nosaukums} style={{ width: 90, height: 68, objectFit: "cover", borderRadius: 6, border: "1px solid #ddd" }} />
            ))}
          </div>
        )}
        ...
        {analīzeData && analīzeData.tips === "dastojums" && (
          <div style={{ marginBottom: 12 }}>
            <button onClick={() => setShowAnalīze(v => !v)}
              style={{ padding: "5px 12px", background: "#f0f8f0", border: "1px solid #225522", borderRadius: 4, cursor: "pointer", fontSize: 12, color: "#225522" }}>
              {showAnalīze ? "▲ Slēpt aprēķinus" : "▼ Rādīt sortimentu aprēķinus"}
            </button>
            {showAnalīze && (
              <div style={{ marginTop: 8, padding: 12, background: "#f0f8f0", borderRadius: 6, border: "1px solid #c8dcc0" }}>
                <div style={{ fontSize: 12, fontWeight: "bold", color: "#225522", marginBottom: 6 }}>Kopējā krāja: {analīzeData.kopKub?.toFixed(1)} m³</div>
                <div style={{ display: "flex", flexWrap: "wrap", gap: 6 }}>
                  {Object.keys(sortimenti).filter(k => sortimenti[k] > 0.1).map(k => (
                    <span key={k} style={{ background: "white", border: "1px solid #c8dcc0", borderRadius: 4, padding: "3px 8px", fontSize: 11 }}>
                      {SORT_NOSAUKUMI[k]}: {sortimenti[k].toFixed(1)} m³
                    </span>
                  ))}
                </div>
                <div style={{ fontSize: 11, color: "#888", marginTop: 6 }}>* Aprēķinu precizitāte atkarīga no inventarizācijas datuma</div>
              </div>
            )}
          </div>
        )}
        ...
        <div style={{ display: "flex", gap: 12, marginBottom: 14, flexWrap: "wrap" }}>
          <div style={{ background: "#f0f8f0", borderRadius: 6, padding: "8px 14px", flex: 1 }}>
            ...
            <div style={{ fontSize: 20, fontWeight: "bold", color: "#225522" }}>{izsole.sakumcena?.toLocaleString()} €</div>
            <div style={{ fontSize: 11, color: "#888" }}>Solis: {solis} €</div>
          </div>
          <div style={{ background: "#fff3e0", borderRadius: 6, padding: "8px 14px", flex: 1 }}>
            ...
            <div style={{ fontSize: 20, fontWeight: "bold", color: "#e65100" }}>{lielakais?.toLocaleString()} €</div>
            <div style={{ fontSize: 11, color: "#888" }}>{soljumi.length} solījum{soljumi.length === 1 ? "s" : "i"} · {dalibnieki.length} dalībnieki</div>
          </div>
          {manamsPedjais && !irAutors && (
            <div style={{ background: "#e8eaf6", borderRadius: 6, padding: "8px 14px", flex: 1 }}>
              ...
              <div style={{ fontSize: 20, fontWeight: "bold", color: "#1565c0" }}>{manamsPedjais?.toLocaleString()} €</div>
              {manamsPedjais >= lielakais
                ? <div style={{ fontSize: 11, color: "#225522" }}>🥇 Vadībā!</div>
                : <div style={{ fontSize: 11, color: "#c62828" }}>Pārsolīts — min. {(lielakais + solis).toLocaleString()} €</div>
              }
            </div>
          )}
        </div>
        ...
        {(faze === "beigusies" || irAutors) && (
          <div style={{ fontSize: 12, color: "#666", marginBottom: 10, padding: "8px 12px", background: "#f9f9f9", borderRadius: 6 }}>
            <b>{izsole.autors}</b>
            {(irAutors || faze === "beigusies") && izsole.autorsTalrunis && <span> · 📞 {izsole.autorsTalrunis}</span>}
            {(irAutors || faze === "beigusies") && izsole.autorsEpasts && <span> · ✉️ {izsole.autorsEpasts}</span>}
          </div>
        )}
                ...
                {soljumi.map((s, i) => (
                  <div key={s.id} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "8px 12px", background: i === 0 ? "#e8f5e9" : i % 2 === 0 ? "white" : "#fafafa", borderBottom: "1px solid #f0f0f0" }}>
                    ...
                    <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
                      <span style={{ fontWeight: "bold", color: i === 0 ? "#225522" : "#333", fontSize: 15 }}>{s.summa?.toLocaleString()} €</span>
        ...
        <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
          {user && !irAutors && faze === "gaida" && !irDalibnieks && (
            <button onClick={() => setShowDalibaNoteikumi(true)}
              style={{ padding: "8px 20px", background: "#1565c0", color: "white", border: "none", borderRadius: 6, cursor: "pointer", fontWeight: "bold" }}>
              ✋ Pieteikties dalībai
            </button>
          )}
          {user && !irAutors && faze === "gaida" && irDalibnieks && (
            <div style={{ padding: "8px 16px", background: "#e8f5e9", border: "1px solid #225522", borderRadius: 6, fontSize: 13, color: "#225522" }}>
              ✓ Dalība apstiprināta — gaidām izsoles sākumu
            </div>
          )}
          {user && !irAutors && faze === "aktiva" && !irDalibnieks && (
            <button onClick={() => setShowDalibaNoteikumi(true)}
              style={{ padding: "8px 20px", background: "#1565c0", color: "white", border: "none", borderRadius: 6, cursor: "pointer" }}>
          ...
          {!user && faze !== "beigusies" && (
            <div style={{ fontSize: 12, color: "#888", padding: "8px 0" }}>🔒 Piesakies lai piedalītos izsolē</div>
          )}
          {irAutors && (
        ...
        <div style={{ position: "absolute", top: "100%", left: 0, right: 0, background: "white", border: "1px solid #ccc", borderRadius: "4px", zIndex: 100, boxShadow: "0 2px 8px rgba(0,0,0,0.15)" }}>
          {piedavajumi.map(n => (
            <div key={n} onClick={() => izveleties(n)}
              ...
              onMouseLeave={e => e.target.style.background = "white"}>{n}</div>
          ))}
        </div>
      )}
    </div>
          ...
          <div style={{ display: "flex", flexWrap: "wrap", gap: "4px", marginTop: "6px" }}>
            {novadi.map(n => (
              <span key={n} style={{ background: "#e8f5e9", border: "1px solid #225522", borderRadius: "4px", padding: "3px 8px", fontSize: "11px", display: "flex", alignItems: "center", gap: "4px" }}>
                {n}<button onClick={() => nonemtNovadu(n)} style={{ background: "none", border: "none", color: "#c62828", cursor: "pointer", fontWeight: "bold", padding: "0", fontSize: "12px" }}>×</button>
              </span>
            ))}
          </div>
        )}
      ...
      <div style={{ marginBottom: "16px" }}>
        ...
        <div style={{ display: "flex", gap: "8px", alignItems: "center" }}>
          ...
          <label style={{ fontSize: "12px", display: "flex", alignItems: "center", gap: "4px" }}>
            <input type="checkbox" checked={cenaPecVienosanas} onChange={e => setCenaPecVienosanas(e.target.checked)} />Pēc vienošanās
          </label>
        </div>
      </div>
      <div style={{ display: "flex", gap: "8px" }}>
        <button onClick={saglabat} style={{ padding: "8px 20px", background: "#225522", color: "white", border: "none", borderRadius: "4px", cursor: "pointer", fontWeight: "bold" }}>💾 Saglabāt</button>
...
export default function SludinajumiPage({ user, onBack }) {
  ...
  useEffect(() => {
    const interval = setInterval(() => { setTick(t => t + 1); setIzsoles(getIzsoles()) }, 30000)
  ...
  return (
    <div style={{ padding: "24px", fontFamily: "Arial", maxWidth: "900px", margin: "0 auto" }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "16px", flexWrap: "wrap", gap: "8px" }}>
        <h2 style={{ color: "#225522", margin: 0 }}>🌲 Sludinājumi & Izsoles</h2>
      ...
      {aktīvāCilne === "sludinajumi" && <>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 12, flexWrap: "wrap", gap: 8 }}>
          <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
            <select value={filtrsDarbiba} onChange={e => setFiltrsDarbiba(e.target.value)} style={{ padding: "6px", border: "1px solid #ccc", borderRadius: "4px", fontSize: "12px" }}>
              <option value="">Visi darbības veidi</option>
              {DARBIBAS_VEIDI.map(d => <option key={d}>{d}</option>)}
            </select>
            <select value={filtrsNovads} onChange={e => setFiltrsNovads(e.target.value)} style={{ padding: "6px", border: "1px solid #ccc", borderRadius: "4px", fontSize: "12px" }}>
              <option value="">Visi novadi</option>
              {NOVADI.map(n => <option key={n}>{n}</option>)}
            </select>
            {(filtrsNovads || filtrsDarbiba) && (
              <button onClick={() => { setFiltrsNovads(""); setFiltrsDarbiba("") }} style={{ padding: "6px 12px", background: "#888", color: "white", border: "none", borderRadius: "4px", cursor: "pointer", fontSize: "12px" }}>✕ Notīrīt</button>
            )}
          </div>
          {user && !mansSludinajums && !showForm && (
            <button onClick={() => setShowForm(true)} style={{ padding: "8px 16px", background: "#225522", color: "white", border: "none", borderRadius: "4px", cursor: "pointer", fontWeight: "bold" }}>➕ Jauns sludinājums</button>
          )}
        </div>
        {!user && <div style={{ background: "#fff8e1", border: "1px solid #f9a825", borderRadius: "6px", padding: "12px", marginBottom: "16px", fontSize: "13px" }}>ℹ️ Lai ievietotu sludinājumu — <b>abonē Meža tirgu</b> vai iegādājies par <b>6.99 €/mēnesī</b></div>}
              ...
              <div key={s.id} style={{ background: "white", border: "1px solid #d0e4c8", borderRadius: "8px", padding: "16px", boxShadow: "0 1px 4px rgba(0,0,0,0.06)" }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: "8px" }}>
                  ...
                  <div style={{ textAlign: "right", fontSize: "12px", color: "#888" }}>
                    <div>{s.datums}</div>
                    <div style={{ color: "#225522", fontWeight: "bold", fontSize: "14px", marginTop: "4px" }}>{s.cenaPecVienosanas ? "Pēc vienošanās" : `${s.cena} €`}</div>
                  </div>
                </div>
                <p style={{ margin: "0 0 8px", fontSize: "13px", color: "#444", lineHeight: "1.5" }}>{s.apraksts}</p>
                <div style={{ display: "flex", flexWrap: "wrap", gap: "4px", marginBottom: "8px" }}>
                  {s.novadi?.map(n => <span key={n} style={{ background: "#f0f6ec", border: "1px solid #c8dcc0", borderRadius: "4px", padding: "2px 6px", fontSize: "11px" }}>📍 {n}</span>)}
                </div>
      ...
      {aktīvāCilne === "izsoles" && <>
        <div style={{ display: "flex", justifyContent: "flex-end", marginBottom: 12 }}>
          {user && !showIzsoleForm && (
            <button onClick={() => setShowIzsoleForm(true)} style={{ padding: "8px 18px", background: "#e65100", color: "white", border: "none", borderRadius: 6, cursor: "pointer", fontWeight: "bold" }}>🏷 Izlikt izsolē</button>
          )}
        </div>
